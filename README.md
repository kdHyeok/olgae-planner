...
**얼개 플래너** — PRD · 기능명세서 · 작업을 **번호 하나로 잇는** 기획 관리 도구. PRD 는 정책·요구사항 같은 표(컬렉션)로,
기능명세서는 트리로, 작업은 칸반으로 다루고, 셋이 `[[P1-14]]` 링크로 서로를 가리킵니다. AI 에이전트는 MCP 로 같은 데이터를 읽고 고칩니다.

## 주요 기능

- **PRD 탭** — 표(컬렉션)의 모음입니다. 기본 `PRD` 표는 섹션 문서(제목 + 마크다운), 그 밖의 표는
  속성(`text` · `md` · `select` · `checkbox` · `relation`)이 열. 왼쪽 목차로 오가고, `+ 표 추가` 로
  템플릿(정책·NFR·미결정·테스트·액터·흐름·타겟 사용자·기기·도메인)이나 빈 표를 만듭니다.
...
- **번호와 링크** — 기능·표 행·작업이 프로젝트 안 **하나의 번호**를 공유하고 `P1-14` 로 표기.
  키(`P1`)는 자동 부여되며 목록의 `키` 버튼으로 바꿀 수 있고(같은 계정 안 중복은 409), 링크는 번호 기준이라 안 깨집니다.
  - 본문의 `[[P1-14]]` 는 **링크 칩**(기능 파랑 · 행 초록 · 없는 번호 빗금). 누르면 탭을 바꿔 이동
  - `관련`(relation) 속성은 `＋` 로 검색해 연결. 대상을 특정 표나 `기능명세서 만` 으로 제한 가능
  - **역참조** — "이 기능을 가리키는 항목" 줄, `참조 N` 배지, 표의 `추적표` 보기. 손으로 쓰는 추적표가 필요 없습니다
...
- **관리자 페이지**(`/admin`, admin 만) — 계정 정보·등급 변경·삭제, 계정별 프로젝트 삭제, 가입 허용 토글과 승인·거절.
  마지막 관리자는 강등·삭제 불가.
- **주소** — `/` 내 프로젝트 · `/project/<slug>` · `/admin`. 프로젝트는 순번 대신 **랜덤 slug** 로 가리킵니다(API 도 같음).
...
│   └── PLAN-collections.md  # PRD 구조화 · 커스텀 표 · 작업판 설계 기록 (결정 D1~D14 · 데이터 모델)
