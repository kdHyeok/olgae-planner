...
- **PRD 탭**: 표(컬렉션)의 모음 — 섹션 문서와 정책·요구사항 같은 표를 왼쪽 목차로 오가며 봅니다.
  셀·본문을 **더블클릭**하면 그 자리에서 편집(Ctrl+Enter 저장 · Esc 취소)
- **기능명세서 탭**
...
- **이미지**
  ...
  - 이미지를 지우면 **본문에 남아 있던 `![](…)` 참조도 함께 정리**됩니다(깨진 그림이 남지 않게).
    응답의 `cleaned` 가 정리한 곳의 수입니다
- **용어 사전**
...
- **마크다운 문법** (PRD·기능 설명·용어 설명 공통)
  ...
  - 강조: `**굵게**`, `*기울임*`, `_기울임_`
  - 제목 `#`~`######`, 불릿 `- `, **번호 목록 `1. `**, 이미지 `![](url)`
  - **중첩 목록**: 앞에 공백 2칸마다 한 단계 (`  - 하위`)
  - **인용 `> `**, **코드 블록**(백틱 세 개로 감싸기 — 안쪽은 문법을 해석하지 않음), **구분선 `---`**
  - `[표시이름](https://...)` → 하이퍼링크
...
- **주소(라우팅)**
  ...
  - 특정 항목으로 바로 열려면 `?item=<번호>` 또는 `?node=<번호>`
...
- **PRD 구조화 · 커스텀 표(컬렉션) · 작업** — 설계와 결정은 [doc/PLAN-collections.md](doc/PLAN-collections.md)
  ...
  - **작업 탭**: 기본 `tasks` 표(제목 · 상태 할일/진행중/완료 · 종류 작업/에픽/이슈 · 상위 작업 · 관련 · 내용)를 칸반으로
  - 좌측 목차의 **`+ 표 추가`** 로 템플릿(정책·NFR·미결정·테스트·액터·흐름·타겟 사용자·기기·도메인) 또는 빈 표를 추가
  - 표 헤더의 **`설정`** 에서 이름·보기(문서/표/칸반)·칸반 열 기준과 **속성을 직접 편집** —
    추가·이름 변경·타입 변경(선택 옵션·관련 대상 표)·순서·삭제. 기본 표(`prd`·`tasks`)는 지울 수 없음
  - 속성을 지우면 모든 행의 그 값도 사라지므로 경고와 두 번 클릭 확인
  - 프로젝트 키는 만들 때 **`P1` · `P2` … 로 자동 부여**되고, 목록의 **`키 ○○`** 버튼으로 바꿉니다
    (`P1-14` → `PLNT-14`). 같은 계정의 다른 프로젝트와 겹치면 409 로 거절합니다.
    링크는 번호 기준이라 키를 바꿔도 깨지지 않습니다
  - 표 설정의 속성 타입에는 아이콘이 붙습니다 — `T` 한 줄 글 · `¶` 마크다운 · `▾` 선택 · `☑` 체크 · `↗` 관련
- **링크 — 기능 ↔ 정책 ↔ 작업**
  - 본문 아무 데나 `[[P1-14]]` 라고 쓰면 **링크 칩**이 됩니다. 기능은 파란색, 표의 행은 초록색,
    없는 번호는 빗금 칩(조용히 사라지지 않게). 누르면 탭까지 바꿔 그 항목으로 이동하고 잠깐 노랗게 표시
  - `관련`(relation) 속성은 `＋` 버튼으로 검색해 연결하고 칩의 `✕` 로 해제. 속성에 대상 표가 지정돼 있으면 그 표만
  - **역참조**: "이 기능을 가리키는 항목" 줄, 표 행의 `참조 N` 배지, 삭제 버튼의 `✕N`.
    표 헤더의 `추적표` 를 누르면 행마다 누가 가리키는지 한눈에 — PRD 9장의 정책 추적표를 손으로 쓰지 않아도 됩니다
  - 주소로도 갑니다 — `?item=<번호>` · `?node=<번호>`
- **작업판 (칸반)**
  - 작업 탭의 표는 상태(할일·진행중·완료)가 열인 **칸반**. 카드를 끌어 상태를 바꿉니다
  - 카드에 종류 배지(`● 작업` · `◈ 에픽` · `! 이슈`), 에픽에는 `하위 3/7` 진행도
  - 행 상세에서 **종류는 맨 위 아이콘 버튼**으로 고릅니다. 새 작업의 기본값은 `작업`
    (선택 속성은 새 행을 만들 때 **첫 옵션**이 기본값이 됩니다)
  - **상위 작업**을 정하면 상대 카드에 **하위 작업**으로 저절로 뜹니다 — 한쪽만 정하면 됩니다.
    하위 작업 쪽에서 `＋` 로 추가해도 같습니다(고른 행의 상위 작업에 내가 들어감)
  - 서로를 상위로 두는 **고리는 막습니다**. 후보 목록에서도 빠지고, 서버도 400 으로 거절
  - 카드를 누르면 **행 상세** — 내용 · **이력**(누가 언제 무엇을 바꿨는지) · **코멘트**
  - `미결정 사항` 표도 `미결정/결정` 칸반으로 보입니다
- **프로젝트 / 멤버**
  ...
  - 참여중인 프로젝트는 `내 프로젝트` 목록에 **소유자 이름과 권한**이 함께 표시되고,
    권한에 따라 버튼이 달라집니다
  - 공유 링크(`/?share=<token>`)는 링크를 아는 누구나 **열람**할 수 있고(편집은 불가), 해제하면 즉시 차단됩니다
...
표 공통 툴 5개로 정책·요구사항·미결정·테스트·**작업**을 모두 다룹니다.
에이전트가 작업 지시를 받으면 `search_items(collection='tasks')` 로 같은 작업을 먼저 찾고,
있으면 `update_item` 으로 상태를 옮기며 진행하고 없으면 `create_item` 으로 만들도록
[스킬](plugin/skills/olgae-planner/SKILL.md)에 규칙을 넣어 두었습니다.

프로젝트 생성·삭제, 공유 링크 관리, 이미지 업로드·삭제, **표·속성 삭제**는 툴로 열지 않았습니다(웹 UI 에서 처리).
